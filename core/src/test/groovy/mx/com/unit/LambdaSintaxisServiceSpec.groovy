...
class LambdaSintaxisServiceSpec extends Specification {
  ...
  @Unroll
  def "Implement unit test with sintaxis3 lambda #_number1 | #_number2 | #_response"() {
    given:
    LambdaSintaxisServiceImpl clazz = new LambdaSintaxisServiceImpl()
    Double number1 = _number1
    Double number2 = _number2
    Double response
    when:
    response = clazz.sintaxis3(number1, number2)
    then:
    response == _response
    where:
    _number1 | _number2 | _response
  ...
  @Unroll
  def "Implement unit test with sintaxis4 lambda #_number1 | #_number2 | #_response"() {
    given:
    LambdaSintaxisServiceImpl clazz = new LambdaSintaxisServiceImpl()
    Double number1 = _number1
    Double number2 = _number2
    Double response
    when:
    response = clazz.sintaxis4(number1, number2)
    then:
    response == _response
    where:
    _number1 | _number2 | _response
